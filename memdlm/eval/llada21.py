...
accelerate launch \
    --num_processes 4 \
    memdlm/eval/llada21.py \
    --tasks gsm8k_cot \
    --model llada21 \
    --apply_chat_template \
    --num_fewshot 5 \
    --model_args "pretrained=ML-GSAI/LLaDA2.1-mini,max_new_tokens=512,steps=32,block_size=32"
"""
...
@dataclass
class LLaDA21EvalConfig(LLaDA2SamplerConfig):
...
class LLaDA21EvalHarness(LM):
    ...
    def __init__(
        self,
        config: LLaDA21EvalConfig | None = None,
        **kwargs,
    ):
        super().__init__()
        if config is None:
            config = LLaDA21EvalConfig()
        ...
        steps_per_block = int(kwargs.get("steps", kwargs.get("steps_per_block", config.steps_per_block)))
    ...
    @staticmethod
    def _make_full_attn_4d(input_ids: torch.Tensor) -> torch.Tensor:
        b, l = input_ids.shape
        return torch.zeros(
            (b, 1, l, l), device=input_ids.device, dtype=torch.bfloat16
        )
    ...
    def _maybe_memory_adapt(self, prompt_ids: torch.Tensor) -> None:
        ...
        attention_mask = self._make_full_attn_4d(prompt_ids)
    ...
    def get_logits(
        ...
        attn_mask = self._make_full_attn_4d(batch)
        logits = self.model(batch, attention_mask=attn_mask).logits
    ...
    def generate_until(self, requests: list[Instance]) -> list[str]:
        out = []
        sampler = LLaDA2Sampler(model=self.model, tokenizer=self.tokenizer)
        ...
        for instance in tqdm(requests, desc="Generating..."):
            ...
            if isinstance(generated_ids, torch.Tensor):
                ...
                gen_tensor = generated_ids.sequences
            generated_answer = self.tokenizer.decode(
                gen_tensor[0][prompt[0].shape[0] :], skip_special_tokens=False
            )
